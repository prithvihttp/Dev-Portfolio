...
export function GitHubActivity() {
    ...
    return (<section className="section-padding relative">
      <div className="mx-auto max-w-7xl px-6">
        <SectionHeading eyebrow="GitHub Activity" title="Consistent contribution" description="Shipping code and learning in public."/>
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="rounded-3xl glass p-6 md:p-8">
          ...
          <div className="overflow-x-auto">
            <div className="flex gap-1">
              {grid.map((col, w) => (<div key={w} className="flex flex-col gap-1">
                  {col.map((lvl, d) => (<motion.div key={d} initial={{ opacity: 0, scale: 0.5 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ duration: 0.2, delay: (w * 7 + d) * 0.003 }} className={`h-3 w-3 rounded-[3px] md:h-3.5 md:w-3.5 ${intensity[lvl]}`}/>))}
                </div>))}
            </div>
          </div>
          ...
          <div className="mt-5 flex items-center justify-end gap-2 text-xs text-muted-foreground">
            <span>Less</span>
            {intensity.map((c, i) => (<span key={i} className={`h-3 w-3 rounded-[3px] ${c}`}/>))}
            <span>More</span>
          </div>
        </motion.div>
      </div>
